...
import {
  getLoanGuarantors, setLoanGuarantors, toGuarantorInputs,
  type GuarantorInput,
...
export function LoanGuarantorsDialog({ loanId, borrowerMemberId, borrowerName, open, onOpenChange, onSaved }: Props) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const res = await setLoanGuarantors(loanId, guarantors)
      if (res.success) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Guarantors{borrowerName ? ` — ${borrowerName}` : ''}</DialogTitle>
        </DialogHeader>
        ...
        {loading ? (
          ...
          <GuarantorFields
            ...
            members={members}
            selfMemberId={borrowerMemberId ?? undefined}
          />
        )}
