...
use crate::db::general_ledger::{GeneralLedger, get_general_ledger};
use crate::error::AppError;
use crate::state::AppState;

/// General ledger — all receipts/vouchers in a date range, categorised by
/// income/expense type for filtering. `from`/`to` are bare ISO dates.
#[tauri::command]
pub fn get_general_ledger_cmd(
    state: State<Mutex<AppState>>,
    from: String,
    to: String,
) -> Result<GeneralLedger, String> {
    ...
    get_general_ledger(conn, &from, &to).map_err(|e: AppError| e.to_string())
}
